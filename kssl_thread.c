...
void set_get_header_state(connection_state *state)
{
    ...
    state->need = KSSL_HEADER_SIZE;    /* |Maj|Min|Length|ID| */
...
int do_ssl(connection_state *state)
{
    ...
    /* 确认SSL通道OK */
    if (!state->connected) {
        if (!SSL_is_init_finished(state->ssl)) {  /* SSL通道已经建立完毕返回1 */
        ...
        state->connected = 1;      /* 设置标识 */
    }

    // Read whatever data needs to be read (controlled by state->need)
    /* 按照自实现协议，读取数据并处理；起始状态在 set_get_header_state() 初始化 */
    while (state->need > 0) {
...
void read_cb(uv_stream_t *s, ssize_t nread, const uv_buf_t *buf)
{
    ...
    if ((nread == UV_EOF) || (nread < 0)) {
        ...
        if (do_ssl(state)) {                 /* 处理消息 */
            write_queued_messages(state);    /* 应答 */
            flush_write(state);
        } else {
            connection_terminate(state->tcp);
        }
